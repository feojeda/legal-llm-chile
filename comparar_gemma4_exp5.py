...
def main():
    ...
    print("Cargando Gemma 4 exp5 entrenado (base + adaptador)...")
    model_trained = PeftModel.from_pretrained(model_base, r"E:\workspace\modelo_gemma4_exp5_entrenado")

    resultados = []
    resultados.append("=" * 60)
    resultados.append("COMPARACION: GEMMA 4 BASE vs GEMMA 4 EXP5 (5 EPOCAS, CORPUS EXPANDIDO)")
    resultados.append("=" * 60)
    ...
    for prompt in prompts:
        ...
        resultados.append("\n[GEMMA 4 EXP5 ENTRENADO]:")
        resultados.append(generar_respuesta(model_trained, tokenizer, prompt))

    output = "\n".join(resultados)
    print(output)

    with open(r"E:\workspace\comparacion_gemma4_exp5.txt", "w", encoding="utf-8") as f:
        f.write(output)

    print("\n\nComparacion guardada en: E:\\workspace\\comparacion_gemma4_exp5.txt")
